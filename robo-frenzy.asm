...
; Page-0 addresses used (for indirect indexed addressing and other things)
    ...
    ; --- Dedicated Zero-Page for BCD Conversion ---
    BCD_Val      = $A2 ; Replaces Val ($25)
    BCD_Val_Hi   = $A3 ; Replaces Val+1 ($26)
    BCD_Res      = $A4 ; Replaces Res ($27)
    BCD_Res_1    = $A5 ; Replaces Res+1 ($28)
    BCD_Res_2    = $A6 ; Replaces Res+2 ($29)
...
right:
    ...
    ; If we are not at the boundary, it's safe to move right.
    jsr ClearCannon     ; Erase player at the old position.
    inc CannonPos       ; Increment X position.
@checkY_right:
    inc CannonYPos
    lda CannonYPos
...
@cap_y_right:
    ...
    sta CannonYPos
@check_for_gear:
    ; This logic checks if a gear should be spawned on the ground.
    lda CannonYPos
    cmp PlayerY_BottomLimit
    bne @continue_right
    lda GearHeld
...
CheckGearCollection:
    ...
    cmp GearPosY
    bne @exit_collection
    jsr ClearGear ; Prevents the gear from being drawn twice.
    jsr Sfx_GearPickup
    lda #1
    sta GearHeld
    lda #0
...
DrawLevelDisplay:
    ...
    ; Print "TIME:"
    ; Y is already at the correct height from the last routine, so just increment it.
    ; X must be reloaded as it is changed by PrintStr.
    lda #<TimeStr
    sta LAB_01
    lda #>TimeStr
    sta LAB_02
...
DrawTimerDisplay:
    ...
    lda #3
    sta TempLoopY   ; Store loop counter (3) in a temp variable
...
@ClearLoop:
    ...
    ; And print it.
    ldx #9
    ; Y is still set from the top of the routine
    lda #YELLOW
    sta Colour
    jsr PrintTimer
    rts
...
DrawShield:
    ; Set the drawing color once for all shield elements
    lda AccentColour
    sta Colour
...
@x_coord_set:
    ...
    ; Draw the bunker/chest using static values.
    ldx #8
    ldy BunkerY
    lda #1
    jsr DrawChar
...
@GenerateNewID:
    ...
    ; We have a new, valid, and different ID. Use it.
    sta ActiveTentacleID
...
@DT_Outer_Loop:
    lda VICRAST
    cmp #40  ; Loop when raster beam is in visible drawing area.
    bcs @DT_Outer_Loop
    stx DT_LOOP_COUNTER
...
@CAT_Outer_Loop:
    ...
    jsr ClearTentacle       ; Call the new optimized routine
@CAT_Next_Tentacle:
    dex
    bpl @CAT_Outer_Loop
    rts
...
BuildRobotPart:
    ...
    ; --- Look up the data for the current robot part ---
    ldx RobotNum
    txa
...
Irq_HandleGameOver:
    ; Play a series of downbeat notes to indicate that the game is over.
